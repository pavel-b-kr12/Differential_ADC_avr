...
static tablestr codetable[TABLESIZE] PROGMEM=
{
0,UNUSEDCHANNEL,1,
1,UNUSEDCHANNEL,1,
2,UNUSEDCHANNEL,1,
...
6,UNUSEDCHANNEL,1,
7,UNUSEDCHANNEL,1,
0,0,1, //TODO2 check is this correct or have to be 0,0,10
1,0,10,
0,0,200,
1,0,200,
2,2,10,
...
const tablestr codetable[TABLESIZE] PROGMEM=
{
0,UNUSEDCHANNEL,1,
1,UNUSEDCHANNEL,1,
UNUSEDCHANNEL,UNUSEDCHANNEL,0,
...
int analogGetCodeAVR(uint8_t pin1,uint8_t pin2,uint16_t gain)
{
 ...
 for(j=0;j<2;j++)
 {
  ...
  {
   if(pgm_read_byte_near(&codetable[i].c1)==pin1 && pgm_read_byte_near(&codetable[i].c2)==pin2 && pgm_read_byte_near(&codetable[i].gain)==gain) return(i);
  }
...
float analogVoltageValuef(float value,int gain)
{
 value=((float)value*(float)voltscale)/((float)gain*512.0);
 return(value);
}
